...
class Function:
    ...
    def solve(self, point = [0,0]):
        ...
        solutions = []
        try:
            for solution in solve(H, tasks=2, dictionary_output=True):
...
if __name__ == '__main__':
    ...
    coeffs_list = list(itertools.product(list(range(-10,11)), repeat = 3))

    while True:
        phi_x = create(random.choice(coeffs_list), random.sample(monomials, 3))
        phi_y = create(random.choice(coeffs_list), random.sample(monomials, 3))
